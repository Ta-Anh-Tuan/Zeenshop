...
mongoose.connect("mongodb+srv://Zeenutt:silentgarden6666@cluster0.s3em7o6.mongodb.net/e-commerce");
...
app.post('/signup',async(req,res)=>{
    ...
    if (check) {
        return res.status(400).json({success:false,error:"Đã tồn tại tài khoản có cùng Email này"})
    }
    let cart = {};
    for (let i = 0; i < 300; i++) {
        cart[i]=0;
...
const fetchUser = async(req,res,next)=>{
    ...
    if (!token) {
        res.status(401).send({errors:"Vui lòng xác thực token hợp lệ"})
    }
    else{
        try {
